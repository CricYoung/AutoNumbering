...
let changeTimeout: NodeJS.Timeout | undefined; // 用於 debounce
 ...
 */
function findLogicalBlockRange(document: vscode.TextDocument, centerLineNum: number, targetIndentation: string): { start: number, end: number } | undefined {
 ...
 */
function renumberLines(editor: vscode.TextEditor, targetIndentation: string): vscode.TextEdit[] { // 不再是 async, 返回 TextEdit[]
 ...
 */
async function moveLines(editor: vscode.TextEditor, direction: "left" | "right") {
    const document = editor.document;
    ...
    const edits: vscode.TextEdit[] = [];
	  let tLineTop=false;
		if(currentIndentation.length == 0) tLineTop=true; // 如果當前行的縮排為0，則表示它是頂部行
 ...
 * @param editor The active text editor.
 */
async function adjustLinesByLevel(editor: vscode.TextEditor) {
    const document = editor.document;
    const edits: vscode.TextEdit[] = [];
 ...
 */
function getAffectedIndentationLevel(document: vscode.TextDocument, changes: readonly vscode.TextDocumentContentChangeEvent[]): string | undefined {
    ...
    if (firstChange.text.includes('\n') && firstChange.range.start.character === 0 && changeLine > 0){
         ...
         const firstNewLineIndent = getIndentation({ text: lines[0], isEmptyOrWhitespace: lines[0].trim() === '' } as vscode.TextLine);
         if (firstNewLineIndent !== '') return firstNewLineIndent;
    ...
    if (targetLineNum >= document.lineCount) { // 如果變更發生在最後一行之後
         if (targetLineNum > 0) return getIndentation(document.lineAt(targetLineNum-1)); // 看最後一行
         else return ""; // 文件是空的
    }
 ...
 * 取得受變更影響的行範圍
 */
function getAffectedLineRange(document: vscode.TextDocument, changes: readonly vscode.TextDocumentContentChangeEvent[]): { start: number, end: number } | undefined {
    if (changes.length === 0) return undefined;
...
export function activate(context: vscode.ExtensionContext) {
    ...
    const textChangeListener = vscode.workspace.onDidChangeTextDocument(event => {
        ...
        changeTimeout = setTimeout(async () => { // **注意：改回 async 以便使用 await editor.edit**
            const editor = vscode.window.activeTextEditor;
            ...
            let targetIndentation: string | undefined = undefined;
             ...
             if (targetIndentation !== undefined) {
                 ...
                 if (blockRange) {
                     ...
                     const filteredEdits = allEdits.filter(edit =>
                         edit.range.start.line >= blockRange.start && edit.range.end.line <= blockRange.end
                     );
                     ...
                     if (filteredEdits.length > 0) {
                         try {
                             const success = await editor.edit(editBuilder => { // **改回 await**
